...
Если ты не запрашивал вход — просто проигнорируй это письмо.
  ...
  try {
    await transporter.sendMail({
      ...
      to: email,
      subject: '🔐 Код для входа в threep',
      text,
      html,
    })
...
// Отправка Magic Link с прямым URL от NextAuth (без промежуточной страницы)
export async function sendMagicLinkDirect(email: string, callbackUrl: string) {
  const magicLink = callbackUrl
...
// Отправка Magic Link (старая версия с промежуточной страницей - оставляем для совместимости)
