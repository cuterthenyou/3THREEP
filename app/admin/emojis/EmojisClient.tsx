    ...
    <div className="px-6 py-6 max-w-2xl mx-auto">
      ...
      <div style={{ background: 'var(--bg-subtle)', border: '1px solid var(--border-soft)', borderRadius: '1rem', padding: '1.25rem', marginBottom: '2rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
          <label style={LABEL_STYLE}>Изображение</label>
          <input
            ref={fileRef}
            type="file"
            accept="image/png,image/gif,image/webp,image/svg+xml"
            onChange={handleUpload}
            style={{ ...INPUT_STYLE, padding: '0.5rem 0.9rem', cursor: 'pointer' }}
            disabled={uploading}
          />
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
          <label style={LABEL_STYLE}>Имя (латиница, цифры, дефис)</label>
          <input
            type="text"
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
        {emojis.length === 0 && (
          <AdminEmptyState>Пока нет кастомных emoji</AdminEmptyState>
        )}
        {emojis.map(emoji => (
